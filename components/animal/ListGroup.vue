<template>
  <v-layout row wrap class="l-user">
    <v-layout row class="user-filter">
      <search-filter ref="campaignSpecialFilter" labelInput="Cari Group Hewan" :dataDropdowns="dataComponentsDropdowns"></search-filter>
    </v-layout>
    <v-container grid-list-lg class="user-content">
      <v-layout row wrap>
        <v-flex xs12 v-show="panelStatus.listCampaignSpecial">
          <panel-expansion idPanel="listCampaignSpecial" title="Detail Group Hewan" cardColor="#ffffff" headerCardColor="white">
            <div slot="header">
            ...
            <div slot="content" style="position: relative; width: 100%;">
              ...
              >
                ...
                <template slot="items" slot-scope="props">
                  <td>
                    <a href="javascript:;" @click="getDetail(props.item.animals_group_id)">
                      {{ props.item.name }}
                    </a>
                  </td>
                  <td>
                  ...
                  <td>
                    <more-action-table :setting="itemsAction" @clickAction="getActionItems($event, props.item.animals_group_id)"></more-action-table>
                  </td>
                </template>
              </v-data-table>
    ...
    >
      <v-card>
        <v-toolbar style="flex: 0 0 auto;" dark class="primary">
          ...
          <v-toolbar-title>Detil Group Hewan</v-toolbar-title>
        </v-toolbar>
...
import * as ANIMAL from '~/store/modules/animalgroup/types'
...
export default {
  ...
  computed: {
    ...mapState({
      entries: state => state.animalGroup.entries,
      entriesDetail: state => state.animalGroup.entriesDetail
    }),
    filters () {
      let data = {
  ...
  methods: {
    ...
    getActionItems (action, id) {
      if (action === 'edit') {
        this.$nuxt.$router.push('/admin/hewan/group/' + id)
      } else if (action === 'delete') {
        this.$dialog({title: 'Hapus Group Hewan', content: 'Anda yakin akan menghapus group hewan ini?'}, () => {
