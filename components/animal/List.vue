    ...
    <v-container grid-list-lg class="user-content">
      <v-layout row wrap>
        <v-flex xs12 v-show="panelStatus.listCampaignSpecial">
          <panel-expansion idPanel="listCampaignSpecial" title="Data Hewan" cardColor="#ffffff" headerCardColor="white">
            <div slot="header">
            ...
            <div slot="content" style="position: relative; width: 100%;">
              ...
              >
                ...
                <template slot="items" slot-scope="props">
                  <td>
                    <a href="javascript:;" @click="getDetail(props.item.animal_id)">
                      {{ props.item.name }}
                    </a>
                  </td>
                  <td>
...
export default {
  ...
  computed: {
    ...mapState({
      entries: state => state.animal.entries,
      entriesDetail: state => state.animal.entriesDetail
    }),
    filters () {
      let data = {
