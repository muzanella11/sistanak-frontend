<template>
  <v-layout row wrap class="l-user">
    <v-layout row class="user-filter">
      <search-filter ref="campaignSpecialFilter" labelInput="Cari Role" :dataDropdowns="dataComponentsDropdowns"></search-filter>
    </v-layout>
    <v-container grid-list-lg class="user-content">
      <v-layout row wrap>
        <v-flex xs12 v-show="panelStatus.listCampaignSpecial">
          <panel-expansion idPanel="listCampaignSpecial" title="Detail Role" cardColor="#ffffff" headerCardColor="white">
            <div slot="header">
            ...
            <div slot="content" style="position: relative; width: 100%;">
              ...
              >
                ...
                <template slot="items" slot-scope="props">
                  <td>
                    <a href="javascript:;" @click="getDetail(props.item.role_id)">
                      {{ props.item.name }}
                    </a>
                  </td>
                  <td>
                  ...
                  <td>
                    <more-action-table :setting="itemsAction" @clickAction="getActionItems($event, props.item.role_id)"></more-action-table>
                  </td>
                </template>
              </v-data-table>
    ...
    >
      <v-card>
        <v-toolbar style="flex: 0 0 auto;" dark class="primary">
          ...
          <v-toolbar-title>Detil Role</v-toolbar-title>
        </v-toolbar>
...
export default {
  ...
  computed: {
    ...mapState({
      entries: state => state.role.entries,
      entriesDetail: state => state.role.entriesDetail
    }),
    filters () {
      let data = {
  ...
  methods: {
    ...mapActions({
      fetchRole: ROLE.FETCH_RESOURCE,
      fetchRoleDetail: ROLE.FETCH_DETAIL,
      deleteRole: ROLE.DELETE_DATA
    }),
    fetchResource () {
      this.isLoading = true
      this.dataMaster = {}
      ...
      this.fetchRole(data).then((response) => {
    ...
    getDetail (id) {
      this.fetchRoleDetail(id).then((response) => {
    ...
    getActionItems (action, id) {
      if (action === 'edit') {
        this.$nuxt.$router.push('/admin/role/' + id)
      } else if (action === 'delete') {
        this.$dialog({title: 'Hapus Role', content: 'Anda yakin akan menghapus role ini?'}, () => {
