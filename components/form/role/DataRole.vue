    ...
    <v-flex offset-xs2 xs8 class="pt-4 pb-4" :class="{'en-loading': isLoading}">
      <v-card flat class="user__card">
        <header class="user__card-header">
          {{ readonly ? 'Detail' : isDetail ? 'Ubah' : 'Tambah' }} Data Role
        </header>
...
export default {
  ...
  methods: {
    ...mapActions({
      createData: ROLE.CREATE_DATA,
      updateData: ROLE.UPDATE_DATA,
      fetchDetail: ROLE.FETCH_DETAIL_DATA
    }),

    ...mapMutations({
      pruneState: ROLE.PRUNE_STATE
    }),
    ...
    create () {
      ...
      this.createData(data).then((response) => {
        ...
        this.$nuxt.$router.push('/admin/role/list')
        this.isLoading = false
      }, (error) => {
        this.isLoading = false
