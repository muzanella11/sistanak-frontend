    ...
    <v-flex offset-xs2 xs8 class="pt-4 pb-4" :class="{'en-loading': isLoading}">
      <v-card flat class="user__card">
        <header class="user__card-header">
          {{ readonly ? 'Detail' : isDetail ? 'Ubah' : 'Tambah' }} Data Group Hewan
        </header>
...
export default {
  ...
  computed: {
    ...mapState({
      entry: state => state.animalGroup.entry
    })
  },
  ...
  methods: {
    ...
    create () {
      ...
      this.createData(data).then((response) => {
        ...
        this.$nuxt.$router.push('/admin/hewan/group/list')
        this.isLoading = false
      }, (error) => {
        this.isLoading = false
