<template>
  <v-layout row wrap class="l-user">
    <v-layout row class="user-filter">
      <search-filter ref="campaignSpecialFilter" labelInput="Cari Nama Environment" :dataDropdowns="dataComponentsDropdowns"></search-filter>
    </v-layout>
            ...
            <div slot="content" style="position: relative; width: 100%;">
              ...
              >
                ...
                <template slot="items" slot-scope="props">
                  ...
                  <td>
                    <more-action-table :setting="itemsAction" @clickAction="getActionItems($event, props.item.environment_id)"></more-action-table>
                  </td>
                </template>
              </v-data-table>
...
export default {
  ...
  computed: {
    ...mapState({
      entries: state => state.environment.entries,
      entriesDetail: state => state.environment.entriesDetail
    }),
    filters () {
      let data = {
  ...
  methods: {
    ...
    fetchResource () {
      ...
      this.fetchEnvironment(data).then((response) => {
    ...
    getDetail (id) {
      this.fetchEnvironmentDetail(id).then((response) => {
    ...
    getActionItems (action, id) {
      if (action === 'edit') {
        ...
        this.$dialog({title: 'Hapus Environment', content: 'Anda yakin akan menghapus environment ini?'}, () => {
          this.deleteEnvironment(id).then(() => {
            this.fetchResource()
